<template>
	<view class="uni-padding-wrap">
		<view class="header">
			...
			<view class="right-content" @tap="onPageTap('chepiaoxinxi')" :style="{fontSize:'32rpx',color:'rgba(255, 255, 255, 1)'}">
				查看更多
			</view>
		</view>
		<view v-if='1 == 1 || 1 == 2' class="uni-product-list" :class="1 == 2 ? 'active' : ''" :style='{"borderRadius":0,"backgroundColor":"rgba(135, 128, 128, 0.34)"}'>
			<view @tap="onDetailTap('chepiaoxinxi',product.id)" :style='{"borderRadius":"12rpx","backgroundColor":"#fff"}' class="uni-product" v-for="(product,index) in homechepiaoxinxilist" :key="index">
				                				<view :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"center"}' class="uni-product-title">{{product.banci}}</view>
		...
		<view class="list" v-if='1 == 3' :style='{"borderRadius":0,"backgroundColor":"rgba(135, 128, 128, 0.34)"}'>
			<view :style='{"borderRadius":"12rpx","backgroundColor":"#fff"}' @tap="onDetailTap('chepiaoxinxi',product.id)" v-for="(product,index) in homechepiaoxinxilist" :key="index" class="listm flex flex-between">
																<view class="listmr">
					<view class="col3 f30 elip mb15" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"center"}'>{{product.banci}}</view>
				</view>
		...
		<!--list end-->
			...
			<view class="right-content" @tap="onPageTap('shiwuzhaoling')" :style="{fontSize:'32rpx',color:'rgba(255, 255, 255, 1)'}">
				查看更多
			</view>
		</view>
		<view v-if='1 == 1 || 1 == 2' class="uni-product-list" :class="1 == 2 ? 'active' : ''" :style='{"borderRadius":0,"backgroundColor":"rgba(135, 128, 128, 0.34)"}'>
			<view @tap="onDetailTap('shiwuzhaoling',product.id)" :style='{"borderRadius":"12rpx","backgroundColor":"#fff"}' class="uni-product" v-for="(product,index) in homeshiwuzhaolinglist" :key="index">
		...
		<view class="list" v-if='1 == 3' :style='{"borderRadius":0,"backgroundColor":"rgba(135, 128, 128, 0.34)"}'>
			<view :style='{"borderRadius":"12rpx","backgroundColor":"#fff"}' @tap="onDetailTap('shiwuzhaoling',product.id)" v-for="(product,index) in homeshiwuzhaolinglist" :key="index" class="listm flex flex-between">
																<view class="listmr">
	...
	export default {
		...
		async onShow() {
			...
			this.news = res.data.list
																					...
																					res = await this.$api.list('chepiaoxinxi', {
				page: 1,
				limit: 4
			});
			this.homechepiaoxinxilist = res.data.list
																								res = await this.$api.list('shiwuzhaoling', {
				page: 1,
				limit: 4
			});
